...
class HamaBrowserActMonitor:
    ...
    def _extract_via_browser_session(self, symbol: str, timeframe: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout:
                ...
                # 匹配带千分位的数字（如 2,131.96）和纯数字
                price_match = re.search(r'(\d{1,3}(?:,\d{3})*(?:\.\d+)?|\d+\.\d+)', title)
                if price_match:
                    try:
    ...
    def monitor_single_symbol(self, symbol: str, timeframe: str = '15m', check_email: bool = True) -> Optional[Dict[str, Any]]:
        """
        监控单个交易对（使用 BrowserAct 会话提取页面标题数据）
        ...
        Returns:
            监控结果
        """
        # 使用 BrowserAct 会话方式（从页面标题获取价格和趋势）
        result = self._extract_via_browser_session(symbol, timeframe)
