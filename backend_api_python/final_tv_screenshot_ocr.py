#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""
截图 TradingView 私有图表 + OCR 识别
"""
import sys
import io
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')

from selenium import webdriver
from selenium.webdriver.chrome.options import Options
from paddleocr import PaddleOCR
import time
import json
import os
import re

print('=' * 70)
print(' ' * 10 + '截图 TradingView 私有图表 + OCR 识别')
print('=' * 70)

# 读取 Cookie
with open('./tradingview_cookies.json', 'r', encoding='utf-8') as f:
    cookie_data = json.load(f)

cookie_string = cookie_data['cookies']
target_url = 'https://cn.tradingview.com/chart/U1FY2qxO/'

print(f'\n目标: {target_url}')
print(f'Cookie: 已加载 ({len(cookie_string)} 字符)\n')

# 配置 Chrome
chrome_options = Options()
chrome_options.add_argument('--headless')
chrome_options.add_argument('--no-sandbox')
chrome_options.add_argument('--disable-dev-shm-usage')
chrome_options.add_argument('--window-size=2560,1440')
chrome_options.add_argument('--proxy-server=http://127.0.0.1:7890')

print('启动浏览器...')
driver = webdriver.Chrome(options=chrome_options)

try:
    # 访问主页
    print('访问 TradingView 主页...')
    driver.get('https://cn.tradingview.com/')
    time.sleep(3)

    # 添加 Cookie
    print('添加 Cookie...')
    for item in cookie_string.split('; '):
        if '=' in item:
            name, value = item.split('=', 1)
            try:
                driver.add_cookie({
                    'name': name,
                    'value': value,
                    'domain': '.tradingview.com',
                    'path': '/'
                })
            except:
                pass

    # 访问目标页面
    print(f'访问目标图表...')
    driver.get(target_url)

    # 等待加载
    print('等待页面加载 (20秒)...')
    time.sleep(20)

    # 截图
    screenshot_path = '../screenshot/TV_Private_Chart_Final.png'
    driver.save_screenshot(screenshot_path)
    file_size = os.path.getsize(screenshot_path)

    print(f'\n✅ 截图成功!')
    print(f'   文件: {screenshot_path}')
    print(f'   大小: {file_size / 1024:.1f} KB\n')

    driver.quit()

    # OCR 识别
    print('=' * 70)
    print('开始 OCR 识别')
    print('=' * 70 + '\n')

    print('初始化 PaddleOCR (中英文混合)...')
    ocr = PaddleOCR(lang='ch')
    print('✅ 就绪\n')

    print('识别图表...\n')
    result = ocr.ocr(screenshot_path)

    if result and len(result) > 0:
        ocr_result = result[0]

        # 获取文本
        texts = []
        if hasattr(ocr_result, 'rec_texts'):
            texts = ocr_result.rec_texts
        elif isinstance(ocr_result, dict):
            for key, value in ocr_result.items():
                if isinstance(value, list) and len(value) > 0 and isinstance(value[0], str):
                    texts = value
                    break

        if texts:
            print(f'✅ 识别到 {len(texts)} 个文本块\n')
            print('=' * 70)
            print('识别结果 (前80个):')
            print('=' * 70 + '\n')

            for i, text in enumerate(texts[:80], 1):
                print(f'{i:2d}. {text}')

            if len(texts) > 80:
                print(f'\n... 还有 {len(texts) - 80} 个文本块')

            # 完整文本
            full_text = ' '.join(texts)
            print(f'\n完整文本 (前1000字符):')
            print('-' * 70)
            print(full_text[:1000])

            # 提取关键信息
            print(f'\n\n关键信息提取:')
            print('=' * 70)

            # 价格数据
            prices = re.findall(r'\d{1,6}[,.]?\d{0,2}', full_text)
            print(f'价格数据: {prices[:30]}')

            # 币种
            crypto_symbols = re.findall(r'[A-Z]{3,6}USDT', full_text.upper())
            if crypto_symbols:
                print(f'币种: {set(crypto_symbols)}')

            # 指标
            indicators = ['HAMA', 'MA', 'EMA', 'BB', 'MACD', 'RSI', 'Volume', 'SMA', 'WMA']
            found = [text for text in texts if any(indicator in text.upper() for indicator in indicators)]
            if found:
                print(f'\n指标相关信息:')
                for item in found[:20]:
                    print(f'  - {item}')

            # 百分比
            percentages = re.findall(r'\d+\.?\d*%', full_text)
            if percentages:
                print(f'\n百分比数据: {percentages[:15]}')

        else:
            print('❌ 无法提取文本')
            if isinstance(ocr_result, dict):
                print('所有键:', list(ocr_result.keys())[:20])
    else:
        print('❌ OCR 识别失败')

except Exception as e:
    print(f'\n❌ 错误: {e}')
    import traceback
    traceback.print_exc()
    driver.quit()

print('\n' + '=' * 70)
print('✅ 完成!')
print('=' * 70)
