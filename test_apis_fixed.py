...
def test_kline():
    ...
    success, data = make_request('POST', '/api/kline', data=kline_data)
    if success:
        print_success("POST /api/kline (K线数据)")
        print(f"  响应: {json.dumps(data, ensure_ascii=False, indent=2)[:300]}...")
    else:
        print_error(f"POST /api/kline - {data}")
...
def test_indicator():
    ...
    # 获取指标列表 - 正确的端点
    success, data = make_request('POST', '/api/indicator/getIndicators',
                                 data={"userid": user_id} if user_id else {})
    if success:
        print_success("POST /api/indicator/getIndicators")
        print(f"  响应: {json.dumps(data, ensure_ascii=False, indent=2)[:300]}...")
    else:
        print_error(f"POST /api/indicator/getIndicators - {data}")
...
def test_backtest():
    ...
    # 获取回测历史 - 正确的端点
    success, data = make_request('POST', '/api/backtest/backtest/history',
                                 data={"page": 1, "pageSize": 10})
    if success:
        print_success("POST /api/backtest/backtest/history")
        print(f"  响应: {json.dumps(data, ensure_ascii=False, indent=2)[:300]}...")
    else:
        print_error(f"POST /api/backtest/backtest/history - {data}")
...
def main():
    """主函数"""
    print(f"\n{Colors.BOLD}QuantDinger API 全接口测试 (修复版){Colors.RESET}")
